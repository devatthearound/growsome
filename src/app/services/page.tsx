...
const Services = () => {
  const [activeMainTab, setActiveMainTab] = useState<MainTabKey>('coaching');
  ...
  const renderContent = (tabId: string) => {
    const tab = mainTabs[activeMainTab].subTabs.find(tab => tab.id === tabId);
        ...
        return (
          <QuestionBox>
            ...
            <FAQList>
              ...
              <FAQItem>
                <FAQTitle>Q2. 개발사에서 코로나 상황을을 제대로 못 받는 경우도 있다던데요?</FAQTitle>
                <FAQAnswer>
  ...
  return (
    <ServicesSection id="services">
      ...
      <ContentWrapper>
        <ContentContainer>
          <SubTabContainer>
            {mainTabs[activeMainTab].subTabs.map((tab) => (
              <SubTab
                key={tab.id}
          ...
          <Content>
            {mainTabs[activeMainTab].subTabs.map((tab, index) => (
              <ContentSection
                key={tab.id}
