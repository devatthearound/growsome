...
async function seedAllBlogPostsFixed() {
  console.log('🌱 그로우썸 블로그 포스트 생성 (이미지 및 수치 수정)');
  ...
  try {
    ...
    // 5. 수정된 블로그 포스트 데이터 (이미지 경로 수정, 현실적인 뷰수/좋아요)
    const allBlogPosts = [
      {
...
<h3>3. 업무 프로세스 자동화</h3>
<p><strong>반복 업무 자동화로 생산성 향상</strong></p>
<ul>
<li>문서 작성 및 정리 자동화</li>
<li>데이터 입력 및 처리 자동화</li>
<li>일정 관리 및 리마인더 시스템</li>
<li>보고서 생성 자동화</li>
</ul>
...
<p>그로우썸과 함께 여러분의 비즈니스에 맞는 AI 자동화 전략을 수립해보세요.</p>
        ...
        is_featured: true,
        is_hero: true,
        thumbnail_url: null, // 이미지 없음으로 설정
        view_count: 87,
        like_count: 9,
...
<h3>3. 성장 실험 프레임워크</h3>
<p><strong>AARRR 퍼넡 최적화</strong></p>
<ul>
...
<p>그로우썸과 함께 한국 시장에 최적화된 SaaS 전략을 수립해보세요.</p>
    ...
    for (let i = 0; i < allBlogPosts.length; i++) {
      ...
      try {
        ...
        console.log(`✅ [${i + 1}/8] ${createdPost.title} (조회수: ${post.view_count}, 좋아요: ${post.like_count})`);
    ...
    console.log('\n📊 수정된 내용:');
    console.log('• 썸네일 이미지: null (이미지 깨짐 방지)');
    console.log('• 조회수 범위: 28-87 (100 이하)');
    console.log('• 좋아요 범위: 2-9 (10개 미만)');
...
if (require.main === module) {
  seedAllBlogPostsFixed()
    .then(() => {
      console.log('\n✅ 모든 블로그 포스트 생성 완료 (이미지 및 수치 수정)');
      process.exit(0);
    })
    .catch((error) => {
      console.error('\n❌ 시딩 실패:', error);
...
export default seedAllBlogPostsFixed;