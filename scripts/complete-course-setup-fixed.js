// scripts/complete-course-setup-fixed.js
// 모든 강의를 올바른 순서와 제목으로 완성 (따옴표 문제 해결)

const { PrismaClient } = require('@prisma/client');

const prisma = new PrismaClient();

// 완전한 강의 목록 (순서대로) - 따옴표 문제 해결
...
async function main() {
  ...
  if (command === 'setup') {
    ...
    console.log('🎯 완전한 강의 설정 도구 (수정버전)');
    console.log('');
    console.log('사용법:');
    console.log('  npm run complete-setup-fixed setup - 모든 강의를 올바른 순서로 설정');
    console.log('');
    console.log('⚠️ 주의: 기존 강의 데이터가 모두 삭제되고 새로 생성됩니다!');
    console.log('💡 실행: npm run complete-setup-fixed setup');
  }
}
